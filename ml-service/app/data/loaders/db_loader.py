...
from app.core.config import POLLUTANTS_TABLE, WEATHER_TABLE
...
def load_pollutants() -> pd.DataFrame:
    df = read_table(POLLUTANTS_TABLE)
    df["time"] = pd.to_datetime(df["time"])
    missing = [c for c in POLLUTANT_COLUMNS if c not in df.columns]
    if missing:
        print(f"[db_loader] Cảnh báo: bảng {POLLUTANTS_TABLE} thiếu cột {missing}")
    keep = ["time"] + [c for c in POLLUTANT_COLUMNS if c in df.columns]
    return df[keep]


def load_weather() -> pd.DataFrame:
    df = read_table(WEATHER_TABLE)
    df["time"] = pd.to_datetime(df["time"])
    missing = [c for c in WEATHER_COLUMNS if c not in df.columns]
    if missing:
        print(f"[db_loader] Cảnh báo: bảng {WEATHER_TABLE} thiếu cột {missing}")
